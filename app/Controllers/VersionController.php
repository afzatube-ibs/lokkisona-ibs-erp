    ...
    private function buildVersionInfo()
    {
        return [
            ...
            'dependencies' => [
                ...
                'v0.4.2.2 Dev DB Activation Helper: /dev-db-activation shows read-only table verification for Groups Aâ€“F, migration apply order, after-apply test flow, and blocked actions â€” no SQL execution',
                'v0.4.2.1 Sprint Merge QA: /database-safety shows sprint merge checklist, dev DB activation guide, and write module QA matrix â€” no live production activation yet',
                ...
                'v0.3.1â€“v0.3.2 Supplier/Business Source Create/Edit: first write services with CSRF and owner-approved whitelist paths',
                ...
                'SELECT only on admin pages â€” no database writes, no user creation, no password change, no role/permission mutation, no invoice creation, no print log creation, no activity log DB write, and no migration apply',
                ...
                'SELECT only on dispatch, return, and payable pages â€” no database writes, no dispatch batch creation, no return receive submit, no payable ledger creation, no supplier payment, no settlement action, and no migration apply',
                ...
                'SELECT only on order workflow â€” no database writes, no order status action, no workflow mutation, no sync/import, and no migration apply from this page',
                ...
                'SELECT only on supplier opening balances â€” no database writes, no payable ledger creation, no launch lock action, and no migration apply from this page',
                ...
                'SELECT only on product control â€” no database writes, no sync, no stock change, no product cost change, and no migration apply from this page',
                ...
                'SELECT only on supplier and business source pages â€” no database writes, no sync, and no migration apply; write services remain a future owner-approved build',
