    ...
    private function buildVersionInfo()
    {
        return [
            ...
            'dependencies' => [
                ...
                'v0.4.2.2 Dev DB Activation Helper: /dev-db-activation shows read-only table verification for Groups AÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“F, migration apply order, after-apply test flow, and blocked actions ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â no SQL execution',
                'v0.4.2.1 Sprint Merge QA: /database-safety shows sprint merge checklist, dev DB activation guide, and write module QA matrix ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â no live production activation yet',
                ...
                'v0.3.1ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“v0.3.2 Supplier/Business Source Create/Edit: first write services with CSRF and owner-approved whitelist paths',
                ...
                'SELECT only on admin pages ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â no database writes, no user creation, no password change, no role/permission mutation, no invoice creation, no print log creation, no activity log DB write, and no migration apply',
                ...
                'SELECT only on dispatch, return, and payable pages ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â no database writes, no dispatch batch creation, no return receive submit, no payable ledger creation, no supplier payment, no settlement action, and no migration apply',
                ...
                'SELECT only on order workflow ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â no database writes, no order status action, no workflow mutation, no sync/import, and no migration apply from this page',
                ...
                'SELECT only on supplier opening balances ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â no database writes, no payable ledger creation, no launch lock action, and no migration apply from this page',
                ...
                'SELECT only on product control ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â no database writes, no sync, no stock change, no product cost change, and no migration apply from this page',
                ...
                'SELECT only on supplier and business source pages ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â no database writes, no sync, and no migration apply; write services remain a future owner-approved build',
