...
try {
    ...
    $routes = @("/login", "/dashboard", "/activity-log", "/roles-permissions", "/database-safety", "/dev-db-activation", "/migration-runner", "/migration-files", "/migration-dry-run", "/migration-approval", "/migration-execution-lock", "/supplier-opening-balances", "/build-queue", "/health", "/version", "/sync-api-settings", "/users", "/suppliers", "/business-sources", "/product-control", "/order-workflow", "/dispatch-reports", "/supplier-payables", "/settlements", "/reports", "/return-receive", "/status-mapping", "/sync-preview", "/invoice-printing", "/supplier-tools", "/manual-orders")
