...
try {
    ...
    $routes = @("/login", "/dashboard", "/activity-log", "/roles-permissions", "/database-safety", "/health", "/version", "/users")
    ...
    if ($versionResponse.Content -notmatch "v0\.1\.6") {
        Fail "Version check failed: /version does not contain v0.1.6."
    }
    Ok "Version"
