...
try {
    ...
    $routes = @("/login", "/dashboard", "/activity-log", "/roles-permissions", "/database-safety", "/health", "/version", "/users", "/suppliers", "/business-sources", "/product-control", "/order-workflow", "/dispatch-reports", "/supplier-payables", "/return-receive")
    ...
    if ($versionResponse.Content -notmatch "v0\.1\.13") {
        Fail "Version check failed: /version does not contain v0.1.13."
    }
    Ok "Version"
