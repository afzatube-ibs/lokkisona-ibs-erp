...
try {
    ...
    $routes = @("/login", "/dashboard", "/activity-log", "/roles-permissions", "/database-safety", "/health", "/version", "/users", "/suppliers", "/business-sources", "/product-control", "/order-workflow")
    ...
    if ($versionResponse.Content -notmatch "v0\.1\.10") {
        Fail "Version check failed: /version does not contain v0.1.10."
    }
    Ok "Version"
