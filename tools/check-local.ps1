...
try {
    ...
    $routes = @("/login", "/dashboard", "/activity-log", "/roles-permissions", "/database-safety", "/health", "/version", "/users", "/suppliers", "/business-sources", "/product-control", "/order-workflow", "/dispatch-reports", "/supplier-payables")
    ...
    if ($versionResponse.Content -notmatch "v0\.1\.12") {
        Fail "Version check failed: /version does not contain v0.1.12."
    }
    Ok "Version"
