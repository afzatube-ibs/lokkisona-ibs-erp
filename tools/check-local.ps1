...
try {
    ...
    $routes = @("/login", "/dashboard", "/activity-log", "/roles-permissions", "/database-safety", "/health", "/version")
    ...
    if ($versionResponse.Content -notmatch "v0\.1\.5") {
        Fail "Version check failed: /version does not contain v0.1.5."
    }
    Ok "Version"
